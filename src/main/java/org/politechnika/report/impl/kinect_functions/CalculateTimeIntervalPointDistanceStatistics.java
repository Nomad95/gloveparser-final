...
public class CalculateTimeIntervalPointDistanceStatistics implements Function<Map<Long, List<KinectDataDto>>, TimeIntervalPointDistanceStatistics> {
    ...
    TerrifyingPointDistanceSetter setter;
    ...
    private void calculateStatsForEveryTimeInterval(Map<Long, List<KinectDataDto>> kinectData, TimeIntervalPointDistanceStatistics statsByTimeInterval) {
    ...
    private PointDistanceStatistics calculateStatsForTimeInterval(List<KinectDataDto> data) {
        List<PointDistance> points = new LinkedList<>();
        for (int i = 1 ; i < data.size() - 1 ; i++) {
            points.add(calculatePoint(data.get(i), data.get(i-1), i));
        }
